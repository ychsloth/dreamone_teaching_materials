...
function LandingScreen({ imageError, onImageError, onEnter }) {
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden flex items-center justify-center p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      <style>{FONT_IMPORT}</style>

      {/* 裝飾用的淡色方塊圖示，純視覺點綴，不影響操作 */}
      <Box className="hidden sm:block absolute -top-8 -left-8 w-40 h-40 text-yellow-100 rotate-12 pointer-events-none" />
      <Box className="hidden sm:block absolute -bottom-10 -right-10 w-48 h-48 text-red-50 -rotate-12 pointer-events-none" />
      <Box className="hidden md:block absolute top-24 right-16 w-14 h-14 text-blue-100 rotate-45 pointer-events-none" />
      <Box className="hidden md:block absolute bottom-24 left-16 w-10 h-10 text-emerald-100 -rotate-6 pointer-events-none" />

      <div className="w-full max-w-3xl text-center relative">
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif" }} className="text-3xl font-bold text-blue-900 mb-1">
          夢想一號魔術方塊學院
        </h1>
        <p className="text-sm text-blue-400 mb-5">學習地圖</p>
        ...
        <p style={{ fontFamily: "'Space Grotesk', sans-serif" }} className="text-lg md:text-xl font-bold text-blue-800 mb-6">
          玩的不只是魔術方塊，更是五顏六色的夢想
        </p>

        <div className="flex flex-wrap items-center justify-center gap-2.5 mb-8">
          <span className="text-xs font-medium bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded-full">🧩 31 種方塊教材</span>
          <span className="text-xs font-medium bg-yellow-50 text-yellow-700 border border-yellow-300 px-3 py-1.5 rounded-full">🎯 6 個認證等級</span>
          <span className="text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1.5 rounded-full">👩‍🏫 專業師資審核</span>
        </div>
        ...
        {!imageError ? (
          <img
            ...
            alt="學習地圖"
            className="w-full rounded-2xl border-2 border-yellow-400 shadow-lg mb-8 object-cover max-h-[70vh]"
            onError={() => {
        ...
        <button
          onClick={onEnter}
          className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 hover:scale-105 text-white font-medium px-8 py-3.5 rounded-xl shadow-lg shadow-red-200 transition transform"
        >
