  ...
  return (
    <div>
      ...
      <div className="space-y-3 max-h-56 overflow-y-auto mb-4 pr-1">
        ...
        {comments.map((c) => (
          <div key={c.id} className="bg-white border border-slate-100 rounded-lg p-3">
            <div className="flex justify-between text-xs mb-1 gap-2">
              <span className="font-medium text-orange-500 truncate">{c.author || '未知使用者'}</span>
              <span className="text-slate-400 shrink-0">{formatTime(c.time)}</span>
            </div>
            <p className="text-sm text-slate-800 break-words">{c.text}</p>
          </div>
        ))}
      </div>
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-400"
        />
        <button
          onClick={submit}
          disabled={sending}
        ...
        {comments.map((c) => (
          <div key={c.id} className="bg-white rounded-md px-2.5 py-1.5 border border-slate-100">
            <div className="flex justify-between text-[11px] text-slate-400 mb-0.5">
              <span className="font-medium text-orange-500">{c.author}</span>
              <span>{formatTime(c.time)}</span>
            </div>
            <p className="text-xs text-slate-800 break-words">{c.text}</p>
          </div>
        ))}
      </div>
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Icon className="w-5 h-5 text-orange-500" />
          <h3 className="font-semibold text-slate-900">{title}</h3>
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-50 text-slate-700">{files.length} 個版本</span>
        </div>
        {canManage && (
          <button
            onClick={onAdd}
              ...
              {isEditing ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditingId(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium py-2 rounded-lg transition">取消</button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 hover:text-orange-500 transition">
                    <ExternalLink className="w-4 h-4 text-orange-500 shrink-0" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-900 truncate">{f.version_label}・{f.note || '未填寫說明'}</p>
                      <p className="text-xs text-slate-400">{resolveAuthorName(f.uploaded_by)}・{formatTime(f.created_at)}</p>
                    </div>
                  </a>
                  {canManage && (
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => startEdit(f)} className="text-xs bg-white border border-slate-300 hover:border-slate-400 text-slate-700 px-2.5 py-1 rounded-md transition">編輯</button>
                      <button
              ...
              {isEditing ? (
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 hover:text-orange-500 transition">
                    <ExternalLink className="w-4 h-4 text-orange-500 shrink-0" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-900 truncate">{f.version_label}</p>
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-900">介紹文章</h3>
        </div>
        {canEdit && !editing && (
      ...
      {editing ? (
        <div className="space-y-3 mb-4">
          <textarea
            ...
            rows={8}
            className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-400"
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-900">最新版美編講義</h3>
        </div>
        {latest && (
      ...
      <div className="relative bg-slate-50 border border-slate-100 text-slate-800 rounded-lg p-6 min-h-[220px] overflow-hidden mb-4">
        <p className="text-sm leading-relaxed text-slate-800 relative z-0">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        {!sent ? (
          ...
          <div className="text-center py-6">
            <CheckCircle2 className="w-14 h-14 text-slate-600 mx-auto mb-3" />
            <p className="font-medium text-lg mb-1 text-slate-900">已成功發送給教材總監</p>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-white border-l border-slate-200 h-full p-6 overflow-y-auto shadow-xl">
        ...
        <p className="text-xs text-slate-400 mb-6">僅 Admin 樹懶老師可見・資料來自 {PROFILES_TABLE} 資料表</p>

        {pending.length > 0 && (
          <>
            <h4 className="text-sm font-semibold text-slate-800 mb-3">待審核用戶（{pending.length}）</h4>
            <div className="space-y-3 mb-8">
              {loading && <p className="text-sm text-slate-400">讀取中...</p>}
              {pending.map((u) => (
                <div key={u.id} className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                  <p className="font-medium text-sm text-slate-900 truncate mb-1">{u.nickname || u.email || u.id}</p>
                  <p className="text-xs text-slate-400 truncate mb-3">{u.email}</p>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => onSetRole(u, 'general_instructor')} className="text-xs bg-white border border-slate-200 hover:border-slate-400 text-slate-700 px-3 py-1.5 rounded-lg transition">設為一般講師</button>
                    <button onClick={() => onSetRole(u, 'internal_partner')} className="text-xs bg-white border border-slate-200 hover:border-slate-400 text-slate-700 px-3 py-1.5 rounded-lg transition">設為內部夥伴</button>
                    <button onClick={() => onSetRole(u, 'admin')} className="text-xs bg-white border border-slate-200 hover:border-slate-400 text-slate-700 px-3 py-1.5 rounded-lg transition">設為管理者</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <h4 className="text-sm font-semibold text-slate-800 mb-3">所有使用者（{allUsers.length}）</h4>
        <div className="space-y-2">
          {allUsers.map((u) => (
            <div key={u.id} className="flex items-center justify-between gap-3 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 flex-wrap">
              <div className="min-w-0 flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-slate-100 overflow-hidden flex items-center justify-center shrink-0">
                  {u.avatar_url ? <img src={u.avatar_url} alt={u.nickname || u.email} className="w-full h-full object-cover" /> : <UserCheck className="w-4 h-4 text-slate-300" />}
                </div>
                <div className="min-w-0">
                  <p className="text-sm text-slate-800 truncate">{u.nickname || u.email || u.id}</p>
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-full max-w-sm text-center">
        <div className="w-14 h-14 rounded-2xl bg-orange-500 flex items-center justify-center mx-auto mb-6 border border-slate-200">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:border-slate-400 hover:shadow-md text-slate-800 font-medium py-3 rounded-xl transition disabled:opacity-50"
        >
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-full max-w-2xl text-center">
        ...
        <p className="text-sm text-slate-400 mb-6">學習地圖</p>
        {!imageError ? (
          <img
            src={LEARNING_MAP_URL}
            alt="學習地圖"
            className="w-full rounded-2xl border border-slate-200 shadow-sm mb-8 object-cover max-h-[70vh]"
            onError={() => {
          ...
          <div className="w-full rounded-2xl border border-slate-200 bg-slate-50 mb-8 py-16 flex flex-col items-center gap-2">
            <Box className="w-10 h-10 text-slate-300" />
            <p className="text-sm text-slate-400">學習地圖圖片尚未讀取到，請確認 Storage 內 manu/learning_map.jpg 是否存在</p>
          </div>
        )}
        <button
          onClick={onEnter}
          className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-medium px-8 py-3.5 rounded-xl transition"
        >
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-full max-w-sm">
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif" }} className="text-xl font-bold text-slate-900 mb-1 text-center">
        ...
        <div className="flex flex-col items-center mb-6">
          <button
            ...
            className="w-24 h-24 rounded-full bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center mb-2"
          >
            {avatarPreview ? <img src={avatarPreview} alt="頭貼預覽" className="w-full h-full object-cover" /> : <Camera className="w-8 h-8 text-slate-300" />}
          </button>
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handlePick} />
          <button onClick={() => fileRef.current && fileRef.current.click()} className="text-xs text-orange-600 hover:text-orange-700 underline">
            {avatarPreview ? '更換頭貼' : '上傳頭貼'}
          </button>
        </div>
        <label className="text-xs font-medium text-slate-700 mb-1 block">暱稱</label>
        <input
          value={nickname}
          onChange={(e) => setNickname(e.target.value)}
          placeholder="輸入您的暱稱"
          className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-900 mb-6 focus:outline-none focus:ring-2 focus:ring-orange-400"
        />
        <button
          onClick={submit}
          disabled={saving}
          className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-medium py-3 rounded-xl transition"
        >
          {saving ? '儲存中...' : '儲存並繼續'}
        </button>
        {mode === 'edit' && onCancel && (
          <button onClick={onCancel} className="w-full text-slate-500 hover:text-slate-700 text-sm py-3">
            返回
          </button>
        )}
      </div>
  ...
  return (
    <div className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <button onClick={onOpenProfile} className="flex items-center gap-2 text-xs bg-white border border-slate-200 hover:bg-slate-50 px-3 py-1.5 rounded-lg transition">
            <div className="w-6 h-6 rounded-full bg-slate-50 overflow-hidden flex items-center justify-center shrink-0">
              {profile.avatar_url ? (
            ...
            {roleMeta && <roleMeta.icon className="w-3.5 h-3.5 text-orange-500" />}
            <span className="text-slate-800 font-medium">{profile.nickname || session.user.email}，老師好</span>
          </button>
          {role === 'admin' && (
            <button
              onClick={onOpenAdmin}
              className="flex items-center gap-1.5 text-xs font-medium bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 px-3 py-1.5 rounded-lg transition"
            >
...
export default function App() {
  ...
  const resolveAndSyncProfile = useCallback(async (authUser) => {
    ...
    // 這行故意留著：如果暱稱一直被重置，先看這裡印出的 data 是不是 null——
    // 如果資料庫裡明明已經有這筆資料，這裡卻印出 null，代表是 RLS SELECT 政策把你自己這筆都擋住了，
    // 不是前端程式碼的問題。
    console.log(`[${PROFILES_TABLE} 讀取結果]`, { userId: authUser.id, data, error });
    if (error) console.error(`[${PROFILES_TABLE} 讀取失敗，可能是 RLS SELECT 政策設定問題]`, error.message, error);

    if (!data) {
      // 用 upsert 而不是 insert：如果資料列其實已經存在（只是剛剛 SELECT 被 RLS 擋住讀不到），
      // insert 會因為 id 重複而整個失敗，導致每次登入都被迫當成新人重填暱稱。
      // upsert 遇到重複的 id 會改成更新，不會出現這個問題。
      const role = targetRole || 'general_instructor';
      const status = targetRole ? 'approved' : 'pending';
      const upsertResult = await supabase
        .from(PROFILES_TABLE)
        .upsert({ id: authUser.id, email, role, status }, { onConflict: 'id' })
        .select('id, email, role, nickname, avatar_url, status')
        .maybeSingle();
      ...
      return upsertResult.data || { id: authUser.id, email, role, status, nickname: null, avatar_url: null };
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900" style={{ fontFamily: "'Inter', sans-serif" }}>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          <p className="font-semibold text-slate-900">{tier.score}分方塊區</p>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="p-3 text-center">
                                <span className="text-sm font-medium text-slate-800">{cube.name}</span>
        ...
        {view === 'cube' && selectedCube && (
          <div>
            <div className="flex items-center gap-2 text-sm text-slate-400 mb-4 flex-wrap">
              ...
              <ChevronRight className="w-3.5 h-3.5" />
              <span className="text-slate-700">{selectedCube.name}</span>
            </div>
                ...
                {role === 'admin' && (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-sm font-medium px-4 py-2.5 rounded-xl transition"
                    >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 h-fit shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-sm font-semibold text-slate-800">權限說明</h4>
                  </div>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] bg-white border border-slate-200 text-sm text-slate-900 px-5 py-3 rounded-xl shadow-xl flex items-center gap-2">
