...
function LearningMapCubeButton({ name, brokenImages, setBrokenImages, onOpenCube }) {
  ...
  return (
    <button
      ...
      title={name}
    >
      <div className="relative w-14 h-14 sm:w-16 sm:h-16 bg-[#1c1c2e] border border-[#2a2a3a] cyber-chamfer-sm overflow-hidden group-hover:border-[#00ff88] group-hover:shadow-[0_0_8px_#00ff8860] transition">
        {imgUrl && !brokenImages[key] ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <Box className="w-6 h-6 text-[#6b7280]" />
          </div>
        )}
        <span className={`absolute bottom-0 left-0 right-0 text-[9px] font-mono text-center py-0.5 ${tier.bg} ${tier.text}`}>
          {tier.score}
        </span>
      </div>
      <span className="text-[10px] font-mono text-[#6b7280] group-hover:text-[#e0e0e0] transition max-w-[4.5rem] truncate">{name}</span>
    </button>
  );
}
...
function LearningMapGrid({ brokenImages, setBrokenImages, onOpenCube }) {
  ...
  return (
    <div className="w-full bg-[#12121a] border border-[#2a2a3a] cyber-chamfer shadow-[0_0_20px_rgba(0,255,136,0.15)] p-5 sm:p-6 text-left">
      <div className="flex items-center gap-3 mb-5">
        <div className="bg-[#00ff88] text-[#0a0a0f] px-3 py-1 cyber-chamfer-sm font-mono uppercase tracking-wider text-sm font-bold">
          魔術方塊學習地圖
        </div>
      ...
      <div className="flex gap-3 mb-4 pl-24">
        {Object.keys(MONTH_GROUP_FLEX).map((months) => (
          <div key={months} style={{ flex: MONTH_GROUP_FLEX[months] }} className="text-center">
            <span className="text-xs font-mono uppercase tracking-wide text-[#00d4ff]">{months} ↑</span>
          </div>
        ))}
      </div>
        ...
        {LEARNING_MAP_ROWS.map((row) => (
          <div key={row.label} className="flex gap-3 items-start">
            <div className="w-24 shrink-0 pt-2">
              <p className="text-xs font-mono uppercase tracking-wide text-[#e0e0e0] leading-tight">{row.label}</p>
            </div>
            {row.groups.map((g, i) => (
              <div
                key={g.months}
      ...
      <div className="relative h-8 mt-6 mb-2 ml-24">
        ...
        {RANK_MARKERS.map((m) => (
          <div key={m.label} className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col items-center" style={{ left: `${m.pos}%` }}>
            ...
            <span className="text-[10px] font-mono text-[#e0e0e0] mt-1 whitespace-nowrap">{m.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mt-6 pt-4 border-t border-[#2a2a3a]">
        <span className="text-[10px] font-mono uppercase tracking-wide text-[#6b7280]">綜合能力認證・方塊個別分數</span>
        {legendColors.map((l) => (
          <span key={l.score} className={`text-[10px] font-mono w-6 h-6 flex items-center justify-center cyber-chamfer-sm ${l.bg} ${l.score === 30 ? 'text-[#0a0a0f]' : 'text-white'}`}>
            {l.score}
          </span>
        ))}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] relative overflow-hidden flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="w-full max-w-6xl text-center relative">
        ...
        <p className="text-sm text-[#6b7280] mb-5">學習地圖</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 mb-8">
          <span className="text-xs font-mono uppercase tracking-wide bg-[#ff3366]/10 text-[#ff3366] border border-[#ff3366]/50 px-3 py-1.5 rounded-full">🧩 31 種方塊教材</span>
          <span className="text-xs font-mono uppercase tracking-wide bg-[#ffee00]/10 text-[#ffee00] border border-[#ffee00]/50 px-3 py-1.5 rounded-full">🎯 6 個認證等級</span>
          <span className="text-xs font-mono uppercase tracking-wide bg-[#00d4ff]/10 text-[#00d4ff] border border-[#00d4ff]/50 px-3 py-1.5 rounded-full">👩‍🏫 專業師資審核</span>
        </div>
