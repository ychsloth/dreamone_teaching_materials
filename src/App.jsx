  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines cyber-grid-bg flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="w-full max-w-sm">
        <h1 style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-xl font-black text-[#00d4ff] uppercase tracking-widest mb-1 text-center">
        ...
        <div className="flex flex-col items-center mb-6">
          <button
            ...
            className="w-24 h-24 rounded-full bg-[#1c1c2e] border border-[#2a2a3a] overflow-hidden flex items-center justify-center mb-2"
          >
            {avatarPreview ? <img src={avatarPreview} alt="頭貼預覽" className="w-full h-full object-cover" /> : <Camera className="w-8 h-8 text-[#6b7280]" />}
          </button>
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handlePick} />
          <button onClick={() => fileRef.current && fileRef.current.click()} className="text-xs font-mono text-[#00ff88] hover:text-[#00ff88] hover:underline">
            {avatarPreview ? '更換頭貼' : '上傳頭貼'}
          </button>
        </div>
        <label className="text-xs font-mono uppercase tracking-[0.2em] text-[#6b7280] mb-1 block">暱稱</label>
        <input
          value={nickname}
          onChange={(e) => setNickname(e.target.value)}
          placeholder="輸入您的暱稱"
          className="w-full bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-3 py-2.5 text-sm text-[#e0e0e0] mb-6 focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
        />
        <button
          onClick={submit}
          disabled={saving}
          className="w-full border-2 border-[#00ff88] text-[#00ff88] bg-transparent cyber-chamfer font-mono uppercase tracking-wider py-3 disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
        >
          {saving ? '儲存中...' : '儲存並繼續'}
        </button>
        {mode === 'edit' && onCancel && (
          <button onClick={onCancel} className="w-full text-[#6b7280] hover:text-[#00ff88] text-sm font-mono py-3 transition">
            返回
          </button>
        )}
        {mode === 'setup' && onBack && (
          <button onClick={onBack} className="w-full text-[#6b7280] hover:text-[#00ff88] text-sm font-mono py-3 transition">
            返回登入頁面
          </button>
        )}
      </div>
