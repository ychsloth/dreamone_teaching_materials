...
function VersionedFileBlock({ title, icon: Icon, files, canManage, canPublish, onAdd, onEdit, onDelete, onPublish, onReview, onPreview, resolveAuthorName }) {
    ...
    <div className="bg-[var(--card)] border border-[var(--border)] cyber-chamfer p-5 ">
      ...
      {published ? (
        <div className="cyber-chamfer-sm overflow-hidden mb-2" style={{ minHeight: 420 }}>
          <DrivePdfViewer category="edited" recordId={published.id} watermark pageNumber={page} onNumPages={setNumPages} session={session} />
          {numPages > 0 && (
...
function DrivePdfViewer({ category, recordId, watermark, pageNumber, onNumPages, session }) {
  ...
  return (
    <div className="relative bg-black flex flex-col items-center justify-center overflow-auto" style={{ minHeight: 420 }}>
      ...
      {watermark && (
        <div className="absolute inset-0 pointer-events-none select-none flex flex-col justify-around opacity-[0.3] overflow-hidden">
...
// 單純放大預覽用，沒有留言功能，給「純預覽」按鈕使用（內部人員跟外部講師都會用到）
function FullscreenPreviewModal({ file, category, kindLabel, watermark, session, onClose }) {
  const [numPages, setNumPages] = useState(0);
  const [page, setPage] = useState(1);
  return (
    <div className="fixed inset-0 bg-black/90 z-[350] flex items-center justify-center p-4">
      <div className="bg-[var(--card)] border-2 border-[#00ff88] cyber-chamfer w-full h-full max-w-[95vw] max-h-[95vh] flex flex-col shadow-[0_0_30px_rgba(0,255,136,0.25)]">
        <div className="flex items-center justify-between gap-3 p-4 border-b border-[var(--border)]">
          <div>
            <h3 className="font-semibold text-[var(--fg)] uppercase tracking-wide font-mono">純預覽・{kindLabel}</h3>
        ...
        <div className="flex-1 overflow-auto">
          <DrivePdfViewer category={category} recordId={file.id} watermark={watermark} pageNumber={page} onNumPages={setNumPages} session={session} />
        </div>
        {numPages > 0 && (
...
function Header({ profile, session, role, onOpenAdmin, onOpenProfile, onLogout, logoError, onLogoError, onGoHome, hasUnseenActivity, onOpenNotif, onOpenAssign }) {
