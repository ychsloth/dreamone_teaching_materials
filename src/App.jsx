...
function VersionedFileBlock({ title, icon: Icon, files, canManage, canPublish, onAdd, onEdit, onDelete, onPublish, onReview, resolveAuthorName }) {
  ...
  const [editForm, setEditForm] = useState({ version_label: '', file_url: '', note: '', page_count: '' });

  const startEdit = (f) => { setEditingId(f.id); setEditForm({ version_label: f.version_label, file_url: f.file_url, note: f.note || '', page_count: f.page_count || '' }); };
            ...
            <div key={f.id} className={`border cyber-chamfer-sm p-4 bg-[var(--muted)] ${f.is_public ? 'border-[#00ff88]' : 'border-[var(--border)]'}`}>
              {isEditing ? (
                <div className="space-y-2">
                  ...
                  />
                  <div>
                    <label className="text-sm font-mono uppercase tracking-wide text-[var(--mutedFg)] mb-1 block">校稿頁數（設定後校稿時會依頁數分頁留言）</label>
                    <input
                      type="number"
                      min="1"
                      value={editForm.page_count}
                      onChange={(e) => setEditForm((s) => ({ ...s, page_count: e.target.value }))}
                      className="w-32 bg-[var(--card)] border border-[var(--border)] cyber-chamfer-sm px-3 py-2 text-base"
                      placeholder="例如：16"
                    />
                  </div>
                  <div className="flex gap-2">
                  ...
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 hover:text-[var(--accentText)] transition">
                    ...
                    <div className="min-w-0">
                      <p className="text-base font-medium text-[var(--fg)] truncate">
                        ...
                        {f.page_count ? <span className="ml-2 text-sm text-[var(--cyanText)] align-middle">共 {f.page_count} 頁</span> : null}
                      </p>
...
function ReviewModal({ file, kindLabel, comments, commentsLoading, onComment, onEditComment, onDeleteComment, onClose, resolveAuthorName, watermark }) {
  ...
  const [activePage, setActivePage] = useState(pageCount > 0 ? 1 : null);
  const [editingId, setEditingId] = useState(null);
  ...
  const visibleComments = allComments.filter((c) => (activePage === null ? true : c.page === activePage));
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="w-full md:w-96 border-t md:border-t-0 md:border-l border-[var(--border)] p-4 flex flex-col overflow-hidden">
            <h4 className="text-sm font-semibold text-[var(--fg)] uppercase tracking-wide font-mono mb-2">
              {pageCount > 0 ? `第 ${activePage} 頁的校稿留言` : '校稿留言'}
            </h4>
...
function AddFileModal({ kindLabel, form, setForm, onClose, onSubmit, submitting, showPageCount }) {
  return (
...
export default function App() {
  ...
  const [uploadForm, setUploadForm] = useState({ version_label: '', file_url: '', note: '', page_count: '' });
  ...
  const addCubeFile = async (category, form) => {
    ...
    if (category === 'draft' || category === 'edited') {
      payload.page_count = form.page_count ? Number(form.page_count) : null;
    }
    setUploading(true);
    ...
    setUploadForm({ version_label: '', file_url: '', note: '', page_count: '' });
    ...
    <div className={`theme-${theme} min-h-screen bg-[var(--bg)] text-[var(--fg)]`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      {showAddFileModal && (
        <AddFileModal
          ...
          showPageCount={showAddFileModal.category === 'draft' || showAddFileModal.category === 'edited'}
        />
      )}
