...
const TIERS = [
  { score: 10, neon: '#00d4ff', cubes: ['1x3x3', '楓葉', '金字塔', '魔錶'] },
  { score: 20, neon: '#ff00ff', cubes: ['2x2x2', '恐龍', '八葉花'] },
  { score: 30, neon: '#00ff88', cubes: ['3x3x3', '2x2x3', '2x3x3', '三階鏡面', '二階鏡面', '二階五魔方', '費雪', '風火輪', '斜轉', '三階齒輪'] },
  { score: 50, neon: '#ffee00', cubes: ['4x4x4', '5x5x5', 'FTO', '五魔方', '二階金字塔', '四階金字塔'] },
  { score: 60, neon: '#ff8800', cubes: ['Square-1', '超級楓葉', '3x3x4'] },
  { score: 70, neon: '#ff3366', cubes: ['6x6x6', '7x7x7', '三階粽子', '軸方塊', '三葉草'] },
];
...
function LandingScreen({ imageError, onImageError, onEnter }) {
  const dotColors = ['#00d4ff', '#ff00ff', '#00ff88', '#ffee00', '#ff8800', '#ff3366'];
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines cyber-grid-bg relative overflow-hidden flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      <style>{FONT_IMPORT}</style>
      ...
      <div className="w-full max-w-3xl text-center relative">
        ...
        <div className="flex items-center justify-center gap-1.5 mb-3">
          {dotColors.map((c, i) => (
            <span key={i} className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: c, boxShadow: `0 0 6px ${c}` }} />
          ))}
        </div>
        ...
        {!imageError ? (
          <img
            ...
            alt="學習地圖"
            className="w-full cyber-chamfer border border-[#2a2a3a] shadow-[0_0_20px_rgba(0,255,136,0.2)] mb-8 object-cover max-h-[70vh]"
            onError={() => {
          ...
          <div className="w-full cyber-chamfer border border-[#2a2a3a] bg-[#1c1c2e] mb-8 py-16 flex flex-col items-center gap-2">
            <Box className="w-10 h-10 text-[#6b7280]" />
            <p className="text-sm text-[#6b7280]">學習地圖圖片尚未讀取到，請確認 Storage 內是否存在：{LEARNING_MAP_URL}</p>
          </div>
        )}
        <button
          onClick={onEnter}
                  ...
                  <div key={tier.score} className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer overflow-hidden ">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className="w-12 h-12 cyber-chamfer-sm flex items-center justify-center font-bold text-lg font-mono bg-transparent border"
                          style={{ borderColor: tier.neon, color: tier.neon, boxShadow: `0 0 6px ${tier.neon}, 0 0 12px ${tier.neon}40` }}
                        >
                          {tier.score}
                        </div>
        ...
        {view === 'cube' && selectedCube && (
          <div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4 bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-6 mb-6 ">
              <div className="flex items-center gap-4">
                <div
                  className="w-14 h-14 overflow-hidden cyber-chamfer-sm flex items-center justify-center shrink-0 border"
                  style={{ borderColor: selectedCube.tier.neon, color: selectedCube.tier.neon, boxShadow: `0 0 6px ${selectedCube.tier.neon}, 0 0 12px ${selectedCube.tier.neon}40` }}
                >
