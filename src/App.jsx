...
const TIERS = [
  ...
  { score: 30, bg: 'bg-amber-400', text: 'text-[#0a0a0f]', cubes: ['3x3x3', '2x2x3', '2x3x3', '三階鏡面', '二階鏡面', '二階五魔方', '費雪', '風火輪', '斜轉', '三階齒輪'] },
  ...
  return (
    <div>
      ...
      <div className="space-y-3 max-h-56 overflow-y-auto mb-4 pr-1">
        ...
        {!loading && comments.length === 0 && <p className="text-sm text-[#6b7280]">尚無留言</p>}
        {comments.map((c) => (
          <div key={c.id} className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm p-3">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-3 py-2 text-sm text-[#e0e0e0] placeholder-[#4b5563] focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
        />
        <button
          onClick={submit}
          disabled={sending}
          className="border-2 border-[#00ff88] text-[#00ff88] bg-transparent px-3 cyber-chamfer-sm flex items-center justify-center disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
        >
  ...
  return (
    <div className="border-t border-[#2a2a3a] pt-3 mt-3">
      <div className="space-y-2 max-h-32 overflow-y-auto mb-2">
        {loading && <p className="text-xs text-[#6b7280]">讀取中...</p>}
        {!loading && comments.length === 0 && <p className="text-xs text-[#6b7280]">這個版本尚無留言</p>}
        {comments.map((c) => (
          <div key={c.id} className="bg-[#12121a] cyber-chamfer-sm px-2.5 py-1.5 border border-[#2a2a3a]">
            ...
            <p className="text-xs text-[#e0e0e0] break-words">{c.text}</p>
          </div>
        ))}
      </div>
      <div className="flex gap-1.5">
        <input
          ...
          className="flex-1 bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
        />
        <button
          onClick={submit}
          disabled={sending}
          className="border-2 border-[#00ff88] text-[#00ff88] bg-transparent px-2.5 cyber-chamfer-sm disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
        >
...
function VersionedFileBlock({ title, icon: Icon, files, comments, canManage, onAdd, onEdit, onDelete, onComment, commentsLoading, resolveAuthorName, commentField }) {
  ...
  return (
    <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-5 ">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Icon className="w-5 h-5 text-[#00ff88]" />
          <h3 className="font-semibold text-[#e0e0e0] uppercase tracking-wide font-mono">{title}</h3>
          <span className="text-xs px-2 py-0.5 rounded-full bg-[#1c1c2e] text-[#e0e0e0]">{files.length} 個版本</span>
        </div>
        {canManage && (
          <button
            onClick={onAdd}
            className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider border-2 border-[#00ff88] text-[#00ff88] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
          >
            <UploadCloud className="w-3.5 h-3.5" /> 上傳新版本
          </button>
        )}
      </div>
      {sorted.length === 0 && <p className="text-sm text-[#6b7280]">尚無版本，請上傳第一筆。</p>}
      <div className="space-y-4">
        {sorted.map((f) => {
          ...
          return (
            <div key={f.id} className="border border-[#2a2a3a] cyber-chamfer-sm p-4 bg-[#1c1c2e]">
              {isEditing ? (
                <div className="space-y-2">
                  <input
                    value={editForm.version_label}
                    onChange={(e) => setEditForm((s) => ({ ...s, version_label: e.target.value }))}
                    className="w-full bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-3 py-2 text-sm"
                    placeholder="版本號"
                  />
                  <input
                  ...
                  />
                  <div className="flex gap-2">
                    <button onClick={saveEdit} className="flex-1 border-2 border-[#00ff88] text-[#00ff88] bg-transparent text-xs font-mono uppercase tracking-wider py-2 cyber-chamfer-sm hover:bg-[#00ff88] hover:text-[#0a0a0f] transition">儲存</button>
                    <button onClick={() => setEditingId(null)} className="flex-1 border border-[#2a2a3a] text-[#6b7280] bg-transparent text-xs font-mono uppercase tracking-wider py-2 cyber-chamfer-sm hover:border-[#e0e0e0] hover:text-[#e0e0e0] transition">取消</button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 hover:text-[#00ff88] transition">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-[#e0e0e0] truncate">{f.version_label}{f.note ? `・${f.note}` : ''}</p>
                      <p className="text-xs text-[#6b7280]">{resolveAuthorName(f.uploaded_by)}・{formatTime(f.created_at)}</p>
                    </div>
                  </a>
                  {canManage && (
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => startEdit(f)} className="text-xs font-mono uppercase tracking-wider border border-[#2a2a3a] text-[#e0e0e0] bg-transparent px-2.5 py-1 cyber-chamfer-sm hover:border-[#00ff88] hover:text-[#00ff88] transition">編輯</button>
                      <button
                        onClick={() => { if (window.confirm('確定要刪除這個版本嗎？')) onDelete(f.id); }}
                        className="text-xs font-mono uppercase tracking-wider border-2 border-[#ff3366]/50 text-[#ff3366] bg-transparent px-2.5 py-1 cyber-chamfer-sm hover:bg-[#ff3366] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#ff3366,0_0_10px_#ff336640] transition"
                      >
                        刪除
                      </button>
                    </div>
  ...
  return (
    <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-5 ">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        {canManage && (
          <button onClick={onAdd} className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider border-2 border-[#00ff88] text-[#00ff88] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition">
            <UploadCloud className="w-3.5 h-3.5" /> 新增
          </button>
        )}
      </div>
      {sorted.length === 0 && <p className="text-sm text-[#6b7280]">尚無檔案</p>}
      <div className="space-y-2">
        {sorted.map((f) => {
          const isEditing = editingId === f.id;
          return (
            <div key={f.id} className="bg-[#1c1c2e] border border-[#2a2a3a] cyber-chamfer-sm p-3">
              {isEditing ? (
                <div className="space-y-2">
                  <input
                    value={editForm.version_label}
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 hover:text-[#00ff88] transition">
                    <ExternalLink className="w-4 h-4 text-[#00ff88] shrink-0" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-[#e0e0e0] truncate">{f.version_label}</p>
                      {f.note && <p className="text-xs text-[#6b7280] truncate">{f.note}</p>}
                    </div>
                  </a>
                  {canManage && (
                    <div className="flex gap-2 shrink-0">
  ...
  return (
    <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-5 ">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        {canEdit && !editing && (
          <button onClick={() => setEditing(true)} className="text-xs font-mono uppercase tracking-wider border-2 border-[#00ff88] text-[#00ff88] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition">
            編輯文章
          </button>
        )}
      </div>
      {editing ? (
        <div className="space-y-3 mb-4">
          <textarea
            ...
            rows={8}
            className="w-full bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-3 py-2 text-sm text-[#e0e0e0] focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
            placeholder="輸入這顆方塊的介紹文章內容..."
          />
          <div className="flex gap-2">
            <button onClick={save} disabled={saving} className="flex-1 border-2 border-[#00ff88] text-[#00ff88] bg-transparent text-sm font-mono uppercase tracking-wider py-2 cyber-chamfer-sm disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] transition">
              {saving ? '儲存中...' : '儲存文章'}
            </button>
            <button
              onClick={() => { setEditing(false); setDraft(article ? article.content : ''); }}
              className="flex-1 border border-[#2a2a3a] text-[#6b7280] bg-transparent text-sm font-mono uppercase tracking-wider py-2 cyber-chamfer-sm hover:border-[#e0e0e0] hover:text-[#e0e0e0] transition"
            >
              取消
            </button>
          </div>
        ...
        <div className="bg-[#1c1c2e] border border-[#2a2a3a] cyber-chamfer-sm p-4 mb-4 min-h-[100px]">
      ...
      {article ? (
        ...
        <p className="text-xs text-[#6b7280]">總監尚未建立文章內容，儲存後即可開放留言。</p>
      )}
    </div>
  );
}
      ...
      {rows.map((_, i) => (
        <div key={i} className="flex justify-around -rotate-[28deg] whitespace-nowrap">
          {Array.from({ length: 4 }).map((_, j) => (
            <span key={j} className="text-[#e0e0e0] font-bold text-lg mx-6">夢想一號內部機密・嚴禁外流</span>
          ))}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-5 ">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        {latest && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-[#1c1c2e] text-[#e0e0e0] border border-[#00d4ff]/40">{latest.version_label}</span>
        )}
      </div>
      <div className="relative bg-[#1c1c2e] border border-[#2a2a3a] text-[#e0e0e0] cyber-chamfer-sm p-6 min-h-[220px] overflow-hidden mb-4">
        <p className="text-sm leading-relaxed text-[#e0e0e0] relative z-0">
      ...
      {latest ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center justify-center gap-2 border-2 border-[#00ff88] text-[#00ff88] bg-transparent text-sm font-mono uppercase tracking-wider py-2.5 cyber-chamfer-sm hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
        >
          <ExternalLink className="w-4 h-4" /> 開啟 Google Drive 講義
        </a>
      ) : (
        <p className="text-sm text-[#6b7280] text-center">尚無可下載的講義連結</p>
      )}
    </div>
  );
}
...
function AddFileModal({ kindLabel, form, setForm, onClose, onSubmit, submitting }) {
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] p-4" onClick={onClose}>
      <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer w-full max-w-md p-6 shadow-[0_0_30px_rgba(0,255,136,0.15)]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-lg flex items-center gap-2 text-[#e0e0e0] uppercase tracking-wide font-mono">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={onSubmit}
            disabled={submitting}
            className="w-full border-2 border-[#00ff88] text-[#00ff88] bg-transparent cyber-chamfer-sm py-2.5 font-mono uppercase tracking-wider disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
          >
            {submitting ? '送出中...' : '送出'}
          </button>
        </div>
      ...
      <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer w-full max-w-md p-6 shadow-[0_0_30px_rgba(0,255,136,0.15)]" onClick={(e) => e.stopPropagation()}>
        {!sent ? (
          <>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={!title.trim()}
                className="w-full border-2 border-[#00ff88] text-[#00ff88] bg-transparent cyber-chamfer-sm py-2.5 font-mono uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
              >
                送出回報
              </button>
            </div>
          ...
          <div className="text-center py-6">
            ...
            <p className="font-medium text-lg mb-1 text-[#e0e0e0]">已成功發送給教材總監</p>
            <p className="text-sm text-[#6b7280] mb-5">樹懶老師將會盡快確認您的回報內容</p>
            <button onClick={onClose} className="border border-[#2a2a3a] text-[#6b7280] bg-transparent px-5 py-2 cyber-chamfer-sm text-sm font-mono uppercase tracking-wider hover:border-[#e0e0e0] hover:text-[#e0e0e0] transition">關閉</button>
          </div>
        )}
      </div>
    </div>
...
function AdminDrawer({ allUsers, onSetRole, onClose, loading }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-[#12121a] border-l border-[#2a2a3a] h-full p-6 overflow-y-auto shadow-[0_0_30px_rgba(0,255,136,0.15)]">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold text-xl flex items-center gap-2 text-[#e0e0e0] uppercase tracking-wide font-mono">
        ...
        <p className="text-xs text-[#6b7280] mb-6">僅 Admin 樹懶老師可見・資料來自 {PROFILES_TABLE} 資料表</p>

        {pending.length > 0 && (
          <>
            <h4 className="text-sm font-semibold text-[#e0e0e0] uppercase tracking-wide font-mono mb-3">待審核用戶（{pending.length}）</h4>
            <div className="space-y-3 mb-8">
              {loading && <p className="text-sm text-[#6b7280]">讀取中...</p>}
              {pending.map((u) => (
                <div key={u.id} className="bg-[#ff00ff]/10 border border-[#ff00ff]/40 cyber-chamfer p-4">
                  <p className="font-medium text-sm text-[#e0e0e0] truncate mb-1">{u.nickname || u.email || u.id}</p>
                  <p className="text-xs text-[#6b7280] truncate mb-3">{u.email}</p>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => onSetRole(u, 'general_instructor')} className="text-xs font-mono uppercase tracking-wider border border-[#2a2a3a] text-[#e0e0e0] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:border-[#00ff88] hover:text-[#00ff88] transition">設為一般講師</button>
                    <button onClick={() => onSetRole(u, 'internal_partner')} className="text-xs font-mono uppercase tracking-wider border border-[#2a2a3a] text-[#e0e0e0] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:border-[#00ff88] hover:text-[#00ff88] transition">設為內部夥伴</button>
                    <button onClick={() => onSetRole(u, 'admin')} className="text-xs font-mono uppercase tracking-wider border border-[#2a2a3a] text-[#e0e0e0] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:border-[#00ff88] hover:text-[#00ff88] transition">設為管理者</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <h4 className="text-sm font-semibold text-[#e0e0e0] uppercase tracking-wide font-mono mb-3">所有使用者（{allUsers.length}）</h4>
        <div className="space-y-2">
          {allUsers.map((u) => (
            <div key={u.id} className="flex items-center justify-between gap-3 bg-[#1c1c2e] border border-[#2a2a3a] cyber-chamfer-sm px-3 py-2 flex-wrap">
              <div className="min-w-0 flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-[#1c1c2e] overflow-hidden flex items-center justify-center shrink-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm text-[#e0e0e0] truncate">{u.nickname || u.email || u.id}</p>
              ...
              <select
                ...
                className="text-xs bg-[#12121a] border border-[#2a2a3a] cyber-chamfer-sm px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
              >
...
function AuthScreen({ onGoogleLogin, authError, authLoading }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines cyber-grid-bg flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="w-full max-w-sm text-center">
        {!logoError ? (
          <img
            src={LOGO_URL}
            alt="夢想一號"
            className="w-14 h-14 cyber-chamfer object-cover mx-auto mb-6 border border-[#2a2a3a]"
            onError={() => {
        ...
        <h1 style={{ fontFamily: "'Orbitron', sans-serif", textShadow: '0 0 10px rgba(0,255,136,0.5)' }} className="text-2xl font-black text-[#00ff88] uppercase tracking-widest mb-1 cyber-glitch">
          夢想一號魔術方塊學院
        </h1>
        <p className="text-sm text-[#6b7280] mb-10">教材管理系統</p>
        <button
          onClick={onGoogleLogin}
          disabled={authLoading}
          className="w-full flex items-center justify-center gap-3 bg-transparent border border-[#2a2a3a] hover:border-[#00ff88] hover:shadow-[0_0_10px_#00ff8840] text-[#e0e0e0] font-mono uppercase tracking-wider py-3 cyber-chamfer transition disabled:opacity-40"
        >
          {authLoading ? <Loader2 className="w-5 h-5 animate-spin text-[#6b7280]" /> : <GoogleIcon />}
          <span>使用 Google 帳號登入</span>
        </button>
        {authError && <p className="text-xs text-[#ff3366] mt-4">{authError}</p>}
        <p className="text-xs text-[#6b7280] mt-8 leading-relaxed">登入後系統將依您的帳號權限自動顯示對應的教材管理畫面。</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines cyber-grid-bg flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="w-full max-w-sm text-center">
        ...
        <h1 style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-xl font-black text-[#ff00ff] uppercase tracking-widest mb-2">審核身份中</h1>
        <p className="text-sm text-[#6b7280] mb-1">{email}</p>
        <p className="text-sm text-[#6b7280] mb-8 leading-relaxed">您的帳號已建立，正等待教材總監開通權限，請耐心等候或直接聯繫總監協助審核。</p>
        <button onClick={onLogout} className="text-sm font-mono text-[#6b7280] hover:text-[#00ff88] hover:underline">登出</button>
      </div>
    </div>
  );
}
...
function LoadingScreen({ label }) {
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines flex flex-col items-center justify-center gap-3">
      <Loader2 className="w-8 h-8 text-[#00ff88] animate-spin" />
      <p className="text-sm text-[#6b7280]">{label || '載入中...'}</p>
    </div>
  );
}
...
function LearningMapCubeButton({ name, brokenImages, setBrokenImages, onOpenCube }) {
  ...
  return (
    <button
      ...
      title={name}
    >
      <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-[#1c1c2e] border border-[#2a2a3a] cyber-chamfer-sm overflow-hidden group-hover:border-[#00ff88] group-hover:shadow-[0_0_8px_#00ff8860] transition">
        ...
        <span className={`absolute bottom-0 left-0 right-0 text-xs font-mono text-center py-0.5 ${tier.bg} ${tier.text}`}>
          {tier.score}
        </span>
      </div>
      <span className="text-xs font-mono text-[#6b7280] group-hover:text-[#e0e0e0] transition max-w-[5.5rem] truncate">{name}</span>
    </button>
  );
}
...
function LearningMapGrid({ brokenImages, setBrokenImages, onOpenCube }) {
  ...
  return (
    <div className="relative w-full bg-[#12121a] border border-[#2a2a3a] cyber-chamfer shadow-[0_0_20px_rgba(0,255,136,0.15)] p-5 sm:p-6 text-left overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center gap-3 mb-5">
          <div className="bg-[#00ff88] text-[#0a0a0f] px-3 py-1.5 cyber-chamfer-sm font-mono uppercase tracking-wider text-base font-bold">
            魔術方塊學習地圖
          </div>
          <span className="text-sm text-[#6b7280] font-mono uppercase tracking-wide">學習地圖</span>
        ...
        <div className="flex gap-3 mb-4 pl-24">
          {Object.keys(MONTH_GROUP_FLEX).map((months) => (
            <div key={months} style={{ flex: MONTH_GROUP_FLEX[months] }} className="text-center">
              <span className="text-sm font-mono uppercase tracking-wide text-[#00d4ff]">{months} ↑</span>
            </div>
          ))}
        </div>
          ...
          {LEARNING_MAP_ROWS.map((row) => (
            <div key={row.label} className="flex gap-3 items-start">
              <div className="w-24 shrink-0 pt-2">
                <p className="text-sm font-mono uppercase tracking-wide text-[#e0e0e0] leading-tight">{row.label}</p>
              </div>
              {row.groups.map((g, i) => (
                <div
                  key={g.months}
                  style={{ flex: MONTH_GROUP_FLEX[g.months] }}
                  className={`flex flex-wrap gap-2 p-2 border-l ${i === 0 ? 'border-l-0' : 'border-[#2a2a3a]'}`}
                >
                  {g.cubes.length === 0 ? (
        ...
        <div className="relative h-8 mt-6 mb-2 ml-24">
          ...
          {RANK_MARKERS.map((m) => (
            <div key={m.label} className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col items-center" style={{ left: `${m.pos}%` }}>
              <span className="w-2.5 h-2.5 rounded-full bg-[#0a0a0f] border-2 border-[#00ff88] shadow-[0_0_6px_#00ff88]" />
              <span className="text-xs font-mono text-[#e0e0e0] mt-1 whitespace-nowrap">{m.label}</span>
            </div>
          ))}
        </div>

        {/* 分數色彩圖例 */}
        <div className="flex flex-wrap items-center gap-3 mt-6 pt-4 border-t border-[#2a2a3a]">
          <span className="text-xs font-mono uppercase tracking-wide text-[#6b7280]">綜合能力認證・方塊個別分數</span>
          {legendColors.map((l) => (
            <span key={l.score} className={`text-xs font-mono w-7 h-7 flex items-center justify-center cyber-chamfer-sm ${l.bg} ${l.score === 30 ? 'text-[#0a0a0f]' : 'text-white'}`}>
              {l.score}
            </span>
          ))}
        </div>
...
function LandingScreen({ imageError, onImageError, onEnter, onOpenCube, brokenImages, setBrokenImages }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] relative overflow-hidden flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      <style>{FONT_IMPORT}</style>
      ...
      <div className="w-full max-w-6xl text-center relative">
        <h1 style={{ fontFamily: "'Orbitron', sans-serif", textShadow: '0 0 10px rgba(0,255,136,0.5)' }} className="text-3xl md:text-4xl font-black text-[#00ff88] uppercase tracking-widest mb-2 cyber-glitch">
          夢想一號魔術方塊學院
        </h1>
        <p
          style={{ fontFamily: "'Orbitron', sans-serif" }}
          className="text-2xl md:text-5xl font-black uppercase tracking-widest text-white mb-6 cyber-rgb-shift cyber-glitch"
        >
          全宇宙最完整的教材系統
        </p>
        ...
        <p style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-lg md:text-xl font-bold text-[#e0e0e0] uppercase tracking-wide mb-6 cyber-rgb-shift">
          玩的不只是魔術方塊，更是五顏六色的夢想
        </p>

        <div className="flex flex-wrap items-center justify-center gap-2.5 mb-8">
          <span className="text-sm font-mono uppercase tracking-wide bg-[#ff3366]/10 text-[#ff3366] border border-[#ff3366]/50 px-3 py-1.5 rounded-full">🧩 31 種方塊教材</span>
          <span className="text-sm font-mono uppercase tracking-wide bg-[#ffee00]/10 text-[#ffee00] border border-[#ffee00]/50 px-3 py-1.5 rounded-full">🎯 6 個認證等級</span>
          <span className="text-sm font-mono uppercase tracking-wide bg-[#00d4ff]/10 text-[#00d4ff] border border-[#00d4ff]/50 px-3 py-1.5 rounded-full">👩‍🏫 專業師資審核</span>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] cyber-scanlines cyber-grid-bg flex items-center justify-center p-6" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="w-full max-w-2xl">
        <h1 style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-4xl md:text-5xl font-black text-[#00d4ff] uppercase tracking-widest mb-3 text-center">
        ...
        <div className="flex flex-col items-center mb-10">
          <button
            ...
            className="w-44 h-44 rounded-full bg-[#1c1c2e] border-2 border-[#2a2a3a] overflow-hidden flex items-center justify-center mb-4 hover:border-[#00ff88] transition"
          >
          ...
          <button onClick={() => fileRef.current && fileRef.current.click()} className="text-lg font-mono text-[#00ff88] hover:text-[#00ff88] hover:underline">
            {avatarPreview ? '更換頭貼' : '上傳頭貼'}
          </button>
        </div>
        <label className="text-base font-mono uppercase tracking-[0.2em] text-[#6b7280] mb-2 block">暱稱</label>
        <input
          value={nickname}
          onChange={(e) => setNickname(e.target.value)}
          placeholder="輸入您的暱稱"
          className="w-full bg-[#12121a] border-2 border-[#2a2a3a] cyber-chamfer-sm px-5 py-4 text-xl text-[#e0e0e0] mb-10 focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
        />
        <button
          onClick={submit}
          disabled={saving}
          className="w-full border-2 border-[#00ff88] text-[#00ff88] bg-transparent cyber-chamfer font-mono uppercase tracking-wider text-xl py-5 disabled:opacity-40 hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
        >
          {saving ? '儲存中...' : '儲存並繼續'}
        </button>
        {mode === 'edit' && onCancel && (
          <button onClick={onCancel} className="w-full text-[#6b7280] hover:text-[#00ff88] text-lg font-mono py-4 transition">
            返回
          </button>
        )}
        {mode === 'setup' && onBack && (
          <button onClick={onBack} className="w-full text-[#6b7280] hover:text-[#00ff88] text-lg font-mono py-4 transition">
            返回登入頁面
          </button>
        )}
      </div>
...
function Header({ profile, session, role, onOpenAdmin, onOpenProfile, onLogout, logoError, onLogoError, onGoHome }) {
  ...
  return (
    <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur border-b border-[#00ff88]/40 shadow-[0_1px_10px_rgba(0,255,136,0.25)]">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
        <button onClick={onGoHome} className="flex items-center gap-3 text-left hover:opacity-80 transition">
          ...
          <div>
            <p style={{ fontFamily: "'Orbitron', sans-serif" }} className="font-bold leading-tight text-[#00ff88] uppercase tracking-wide text-sm">夢想一號魔術方塊學院</p>
            <p className="text-xs text-[#6b7280] leading-tight">教材管理系統</p>
          </div>
        </button>
        <div className="flex items-center gap-3">
          <button onClick={onOpenProfile} className="flex items-center gap-2 text-xs bg-[#12121a] border border-[#2a2a3a] hover:bg-[#00ff88]/10 px-3 py-1.5 cyber-chamfer-sm transition">
            <div className="w-6 h-6 rounded-full bg-[#1c1c2e] overflow-hidden flex items-center justify-center shrink-0">
              {profile.avatar_url ? (
          ...
          {role === 'admin' && (
            <button
              onClick={onOpenAdmin}
              className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider border border-[#2a2a3a] text-[#e0e0e0] bg-transparent px-3 py-1.5 cyber-chamfer-sm hover:border-[#00ff88] hover:text-[#00ff88] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
            >
...
export default function App() {
  ...
  if (view === 'profile') {
    return (
      <div className="min-h-screen bg-[#0a0a0f]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
        <style>{FONT_IMPORT}</style>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-[#e0e0e0]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      <style>{FONT_IMPORT}</style>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {view === 'dashboard' && (
          <div>
            <h1 style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-2xl font-black mb-1 text-[#e0e0e0] uppercase tracking-widest">
              綜合能力認證分數地圖
            </h1>
            <p className="text-[#6b7280] text-sm mb-8">依 31 顆魔術方塊的認證分數分類，點擊分數展開對應方塊清單</p>
            <div className="space-y-4">
              {TIERS.map((tier) => {
                const isOpen = openTier === tier.score;
                return (
                  <div key={tier.score} className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer overflow-hidden ">
                    <button
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 cyber-chamfer-sm flex items-center justify-center font-bold text-lg font-mono ${tier.bg} ${tier.text}`}>
                          {tier.score}
                        </div>
                        <div className="text-left">
                          <p className="font-semibold text-[#e0e0e0] uppercase tracking-wide font-mono">{tier.score}分方塊區</p>
                          <p className="text-xs text-[#6b7280]">共 {tier.cubes.length} 顆方塊</p>
                        </div>
                      </div>
                        ...
                        {tier.cubes.map((name) => {
                          ...
                          return (
                            <button
                              ...
                              className="group bg-[#12121a] hover:shadow-[0_0_15px_rgba(255,51,102,0.3)] border border-[#2a2a3a] hover:border-[#ff3366] cyber-chamfer overflow-hidden flex flex-col transition text-left"
                            >
                              <div className="aspect-square bg-[#1c1c2e] overflow-hidden flex items-center justify-center">
                              ...
                              <div className="p-3 text-center">
                                <span className="text-sm font-medium text-[#e0e0e0]">{cube.name}</span>
        ...
        {view === 'cube' && selectedCube && (
          <div>
            <div className="flex items-center gap-2 text-sm text-[#6b7280] mb-4 flex-wrap">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4 bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-6 mb-6 ">
              <div className="flex items-center gap-4">
                <div
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 style={{ fontFamily: "'Orbitron', sans-serif" }} className="text-xl font-black text-[#e0e0e0] uppercase tracking-wide">{selectedCube.name}</h2>
                    {canManageFiles && <CubeBadges status={detailStatus} />}
                  </div>
                  <p className="text-xs text-[#6b7280]">認證分數 {selectedCube.tier.score} 分・{selectedCube.tier.score}分方塊區</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {role === 'admin' && (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 border border-[#2a2a3a] text-[#e0e0e0] bg-transparent text-sm font-mono uppercase tracking-wider px-4 py-2.5 cyber-chamfer hover:border-[#00ff88] hover:text-[#00ff88] transition"
                    >
                ...
                {role === 'general_instructor' && (
                  <button
                    ...
                    className="flex items-center gap-1.5 border-2 border-[#00ff88] text-[#00ff88] bg-transparent text-sm font-mono uppercase tracking-wider px-4 py-2.5 cyber-chamfer hover:bg-[#00ff88] hover:text-[#0a0a0f] hover:shadow-[0_0_5px_#00ff88,0_0_10px_#00ff8840] transition"
                  >
            ...
            {role === 'general_instructor' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-[#12121a] border border-[#2a2a3a] cyber-chamfer p-5 h-fit ">
                  <div className="flex items-center gap-2 mb-3">
                    <Lock className="w-4 h-4 text-[#6b7280]" />
                    <h4 className="text-sm font-semibold text-[#e0e0e0] uppercase tracking-wide font-mono">權限說明</h4>
                  </div>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] bg-[#12121a] border border-[#00ff88]/50 text-sm text-[#e0e0e0] font-mono px-5 py-3 cyber-chamfer shadow-[0_0_20px_rgba(0,255,136,0.3)] flex items-center gap-2">
