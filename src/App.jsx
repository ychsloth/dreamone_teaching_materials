...
function VersionedFileBlock({ title, icon: Icon, files, comments, canManage, canPublish, onAdd, onEdit, onDelete, onComment, onPublish, onReview, commentsLoading, resolveAuthorName, commentField }) {
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] cyber-chamfer p-5 ">
      ...
      <div className="space-y-4">
        {sorted.map((f) => {
          const fileComments = comments.filter((c) => c[commentField] === f.id).map((c) => ({ id: c.id, author: resolveAuthorName(c.user_email), text: c.content, time: c.created_at, page: c.page_number }));
          ...
          return (
            <div key={f.id} className={`border cyber-chamfer-sm p-4 bg-[var(--muted)] ${f.is_public ? 'border-[#00ff88]' : 'border-[var(--border)]'}`}>
              ...
              <FileCommentThread comments={fileComments} loading={commentsLoading} showPageInput onAdd={(text, page) => onComment(f.id, text, page)} />
            </div>
          );
        })}
      </div>
...
function InstructorHandout({ files }) {
  ...
  const embedUrl = published ? (() => {
    const fileId = parseDriveFileId(published.file_url);
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] cyber-chamfer p-5 ">
      ...
      {published && embedUrl ? (
        <div className="relative bg-black cyber-chamfer-sm overflow-hidden mb-2" style={{ height: '70vh', minHeight: 420 }}>
          <iframe src={embedUrl} className="w-full h-full border-0" allow="autoplay" title="美編講義預覽" />
...
// 從 Google Drive 分享連結解析出檔案 ID，組出可以直接內嵌預覽的網址
...
function ReviewModal({ file, kindLabel, comments, commentsLoading, onComment, onClose, resolveAuthorName, watermark }) {
  const fileId = parseDriveFileId(file.file_url);
  const embedUrl = fileId ? `https://drive.google.com/file/d/${fileId}/preview` : null;
  const fileComments = comments
    .map((c) => ({ id: c.id, author: resolveAuthorName(c.user_email), text: c.content, time: c.created_at, page: c.page_number }));
      ...
      <div className="bg-[var(--card)] border-2 border-[#00ff88] cyber-chamfer w-full max-w-6xl h-[90vh] flex flex-col shadow-[0_0_30px_rgba(0,255,136,0.25)]">
        <div className="flex items-center justify-between gap-3 p-4 border-b border-[var(--border)] flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ExternalLink className="w-3.5 h-3.5" /> 在 Google Drive 開啟
            </a>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          <div className="relative flex-1 bg-black min-h-[300px]">
            {embedUrl ? (
              <iframe
                ...
                allow="autoplay"
                title={`${file.version_label} 預覽`}
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-[var(--mutedFg)] text-sm">
                無法解析這個連結的檔案 ID，請確認是不是 Google Drive 的分享連結
              </div>
            )}
            {watermark && (
              <div className="absolute inset-0 pointer-events-none select-none flex flex-col justify-around opacity-[0.16] overflow-hidden">
          ...
          <div className="w-full md:w-96 border-t md:border-t-0 md:border-l border-[var(--border)] p-4 flex flex-col overflow-hidden">
            <h4 className="text-sm font-semibold text-[var(--fg)] uppercase tracking-wide font-mono mb-2">逐頁校稿留言</h4>
            <div className="flex-1 overflow-y-auto space-y-2 mb-2">
              {commentsLoading && <p className="text-sm text-[var(--mutedFg)]">讀取中...</p>}
              {!commentsLoading && fileComments.length === 0 && <p className="text-sm text-[var(--mutedFg)]">尚無留言，開始校稿並在下方標註頁碼留言吧。</p>}
              {fileComments
                .slice()
            ...
            <FileCommentThreadInput onAdd={onComment} />
          </div>
        </div>
      </div>
    </div>
...
// ReviewModal 專用的留言輸入框（跟 FileCommentThread 共用邏輯，但不需要重複顯示留言列表）
  ...
  return (
    <div className="flex gap-1.5 shrink-0">
      <input
        ...
        placeholder="頁"
        title="這則留言對應的頁碼（選填）"
        className="w-14 bg-[var(--muted)] border border-[var(--border)] cyber-chamfer-sm px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-[#00ff88]"
      />
      <input
        value={text}
        onChange={(e) => setText(e.target.value)}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <VersionedFileBlock
                    ...
                    onComment={(fileId, text, page) => postFileComment('draft', fileId, text, page)}
                  ...
                  <VersionedFileBlock
                    ...
                    onComment={(fileId, text, page) => postFileComment('edited', fileId, text, page)}
